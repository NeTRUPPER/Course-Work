...
- Подготовьте CSV из Excel (UTF‑8):
  1) Откройте файл `прайс ПРОКАТ ВСЕГО 01, 11,24.xlsx`.
  2) Файл → Сохранить как → CSV (разделители `;` или `,`), кодировка UTF‑8.
  3) Убедитесь, что первая строка содержит заголовки.
- Поддерживаемые названия колонок (можно RU/EN):
  - Название: `Название`, `Наименование`, `Имя`, `Name`, `Item`, `Equipment` (обязательно)
  - Категория: `Категория`, `Category` (обязательно)
  - Цена/день: `Цена`, `Стоимость`, `Price`, `Cost`, `Price per day` (обязательно)
  - Цена за каждый последующий день: `Стоимость вторых суток`, `Вторые сутки`, `Доп. сутки`, `2-е сутки`, `Additional day price`, `Extra day price` (необязательно)
  - Залог: `Залог`, `Deposit` (необязательно)
  - Количество: `Количество`, `Qty`, `Quantity`, `Count` (необязательно)
  - Описание: `Описание`, `Description`, `Notes` (необязательно)
- Импорт в приложении: Меню Файл → «Импорт оборудования (CSV)…» → выберите CSV → дождитесь отчёта (сколько импортировано/пропущено). Дубли по имени/категории не объединяются автоматически.
